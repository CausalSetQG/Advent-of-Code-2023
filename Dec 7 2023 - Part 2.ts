...
function identifyHandType(cards: string[]) {
    let counts = new Map();
    // Keeps track of number of jokers separately and adjust logic for each hand type accordingly
    let joker_count = 0;
    ...
    if (frequencies[0] + joker_count === 5 || joker_count === 5) {
        return 'five_of_a_kind';
    } else if (frequencies[0] + joker_count === 4) {
        return 'four_of_a_kind';
    } else if (frequencies[0] + joker_count === 3 && frequencies[1] === 2) {
        return 'full_house';
            // 3 Jokers always at least 4 of a kind
            // 2 Jokers + Pair is 4 of a kind
            // 2 Jokers + XYZ is 2 pair
            // Therefore full house must have 0 or 1 jokers
            // 1 Joker: XXX YJ is four of a kind, so only possibility is XXJ YY
    } else if (frequencies[0] + joker_count === 3) {
        return 'three_of_a_kind';
    } else if (frequencies[0] === 2 && frequencies[1] === 2) {
        return 'two_pair';
            // 2 Jokers is always at least 3 of a kind
            // 1 Joker can never be 2 pair as it would be 3 of a kind
    } else if (frequencies[0] + joker_count === 2) {
        return 'one_pair';
    } else {
        return 'high_card';
    }}
